...
class SentencePieceTrainer:
    ...
    def train_sentencepiece_model(self, input_file):
        ...
        os.rename(f"{self.model_prefix}.vocab", os.path.join(self.output_dir, f"{self.model_prefix}.vocab"))
        os.rename(f"{self.model_prefix}.model", os.path.join(self.output_dir, f"{self.model_prefix}.model"))

        return output_model_path

    def run(self):
        parser = argparse.ArgumentParser(description="Train a SentencePiece model.")
        parser.add_argument("-input-file", required=True, help="Path to the input text corpus file.")
        parser.add_argument(
            "-output-dir",
        ...
        parser.add_argument("-vocab-size", type=int, default=self.vocab_size, help="Size of the vocabulary.")
        parser.add_argument(
            "-character-coverage",
            type=float,
